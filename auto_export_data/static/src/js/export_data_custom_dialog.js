...
class CustomDeleteExportListDialog extends Component {
    static components = {Dialog};
    static template = "auto_export_data.CustomDeleteExportListDialog";
    static props = {
        text: String,
        close: Function,
        delete: Function,
...
class CustomExportDataItem extends Component {
    static template = "auto_export_data.CustomExportDataItem";
    static components = {CustomExportDataItem};
    static props = {
...
export class CustomExportDataDialog extends Component {
    static template = "auto_export_data.MyFieldPickerDialog";
    static components = {CheckBox, Dialog, CustomExportDataItem};
    static props = {
        close: {type: Function},
    ...
    async loadFields(id, preventLoad = false) {
        ...
        // Ensure fields is iterable (array)
    ...
    async onDeleteExportTemplate() {
        this.dialog.add(CustomDeleteExportListDialog, {
    ...
    async setDefaultExportList() {
        this.state.exportList = Object.values(this.knownFields).filter(
            (e) =>
                e.default_export ||
                this.props.defaultExportList.find((i) => i.name === e.id)
        );
    }
    ...
    onClose() {
        // Always restore the previous action (form view) when dialog closes
        if (this.props.close) {
            this.props.close();
        }
