...
class HrEmployee(models.Model):
    ...
    def get_department_leave(self):
        ...
        if leave_lines:
            ...
            if pd is not None:
                ...
                # Group leave lines by month and department manually
                grouped_data = {}
                for line in leave_lines:
                    key = (line['l_month'], line['department'])
                    if key not in grouped_data:
                        grouped_data[key] = 0
                    grouped_data[key] += line['days']
                
                # Apply the grouped data to the graph result
                for (month, dept_id), days in grouped_data.items():
                    # Find the department name
                    dept_name = self.env['hr.department'].browse(dept_id).name
                    # Find the matching month in graph_result
                    for result in graph_result:
                        if result['l_month'].replace(' ', '') == month.replace(' ', ''):
                            result['leave'][dept_name] = days
                            break
        for result in graph_result:
    ...
    def employee_leave_trend(self):
        ...
        if leave_lines:
            ...
            if pd is not None:
                ...
                # Fallback implementation without pandas
                # Group leave lines by month manually
                grouped_data = {}
                for line in leave_lines:
                    month = line['l_month']
                    if month not in grouped_data:
                        grouped_data[month] = 0
                    grouped_data[month] += line['days']
    ...
    def join_resign_trends(self):
        ...
        try:
            ...
            # If there's any SQL error, continue with empty data
            pass
    ...
    def get_attrition_rate(self):
        ...
        # Safely extract month_join and month_resign with default empty lists
        month_join = []
        month_resign = []
        if isinstance(monthly_join_resign, list) and len(monthly_join_resign) >= 2:
            if 'values' in monthly_join_resign[0]:
                month_join = monthly_join_resign[0]['values']
            if 'values' in monthly_join_resign[1]:
        ...
        for month_date in month_start_list:
            ...
            # Safely get join count with default value 0
            join_matches = list(filter(
                lambda d: d['l_month'] == month_emp[1].split(' ')[:1][
                                              0].strip()[:3], month_join)) if month_join else []
            match_join = join_matches[0]['count'] if join_matches else 0
            
            # Safely get resign count with default value 0
            resign_matches = list(filter(
                lambda d: d['l_month'] == month_emp[1].split(' ')[:1][
                                              0].strip()[:3],
                month_resign)) if month_resign else []
            match_resign = resign_matches[0]['count'] if resign_matches else 0
