...
class PosOrder(models.Model):
    ...
    def get_the_top_customer(self):
        ...
        company_id = self.env.company.id
        query = '''select res_partner.name as customer,sum(pos_order.amount_paid) as amount_total from pos_order 
        inner join res_partner on res_partner.id = pos_order.partner_id where pos_order.company_id = ''' + str(
            company_id) + ''' and pos_order.partner_id is not null GROUP BY pos_order.partner_id, res_partner.name  ORDER BY amount_total  DESC LIMIT 10;'''
        self._cr.execute(query)
        docs = self._cr.dictfetchall()
        ...
        customers = []
        for record in docs:
            amounts.append(record.get('amount_total'))
            customers.append(record.get('customer'))
        
        # Handle case where there is no customer data
        if not amounts and not customers:
            amounts = [0]
            customers = ['No Data']
            
        final = [amounts, customers]
        return final
