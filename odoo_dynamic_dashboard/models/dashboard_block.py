    ...
    def get_dashboard_vals(self, action_id, start_date=None, end_date=None):
        ...
        for rec in self.env['dashboard.block'].sudo().search(
            ...
            if 'company_id' in self.env[rec.model_name]._fields:
                # Use the actual table name instead of the model name
                table_name = self.env[rec.model_name]._table
                domain = expression.AND([[('%s.company_id' % table_name, '=', self.env.company.id)], domain])
                # ✅ Add current user domain (if relevant)
            if 'user_id' in self.env[rec.model_name]._fields:
                # Use the actual table name instead of the model name
                table_name = self.env[rec.model_name]._table
                domain = expression.AND([[('%s.user_id' % table_name, '=', self.env.user.id)], domain])
