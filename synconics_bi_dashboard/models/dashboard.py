...
class Dashboard(models.Model):
    ...
    def get_charts_details(self):
        ...
        # Get charts directly assigned to this dashboard
        charts = self.chart_ids
        
        # If this is the "My Dashboard", show all charts
        if self.name == "My Dashboard":
            # Get all charts in the system
            all_charts = self.env["dashboard.chart"].search([])
            _logger.info("My Dashboard detected. Total charts in system: %s", len(all_charts))
            charts = all_charts
        
        for chart in charts:
            _logger.info("Checking chart: %s (ID: %s, Type: %s)", chart.name, chart.id, chart.chart_type)
            if (
                ...
                _logger.info("Skipping chart %s due to access restrictions", chart.name)
                continue
            _logger.info("Including chart: %s (ID: %s)", chart.name, chart.id)
        ...
        _logger.info("Returning %s charts for dashboard %s", len(chart_data_list), self.name)
        return [int(self.auto_reload_duration), chart_data_list, self.name]

    @api.model
    def get_or_create_my_dashboard(self, user):
        """
        Get or create a "My Dashboard" for the specified user
        """
        # First try to find a dashboard specifically for this user
        my_dashboard = self.search([
            ("name", "=", "My Dashboard"),
            ("user_ids", "in", user.id)
        ], limit=1)
        
        # If not found, try to find a general "My Dashboard"
        if not my_dashboard:
            my_dashboard = self.search([
                ("name", "=", "My Dashboard"),
        ...
        if not my_dashboard:
            my_dashboard = self.create({
                "name": "My Dashboard",
                "user_ids": [(4, user.id)],
                "access_by": "user"
            })
            # Create the menu for this dashboard
            my_dashboard.create_update_menu()
            
            # Automatically add all existing charts to this user's dashboard
    ...
    def action_add_all_charts_for_user(self):
        """
        Add all existing charts to this dashboard for the current user
        """
        self.ensure_one()
        all_charts = self.env["dashboard.chart"].search([])
        for chart in all_charts:
            if chart.dashboard_id.id != self.id:
                chart.write({
                    "dashboard_id": self.id,
                    "worked_by_user_ids": [(4, self.env.user.id)]
                })
        return True
...
class DashboardMail(models.Model):
    ...
    def _default_mail_template(self):
        ...
        """
        template = self.env.ref("synconics_bi_dashboard.mail_template_dashboard_chart", raise_if_not_found=False)
