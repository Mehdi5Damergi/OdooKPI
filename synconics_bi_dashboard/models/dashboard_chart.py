...
class DashboardChart(models.Model):
    ...
    def create(self, vals_list):
        records = super(DashboardChart, self).create(vals_list)
        # Track the user who created the chart
        for record in records:
            if self.env.user:
                record.worked_by_user_ids = [(4, self.env.user.id)]
                # Also add this chart to the user's "My Dashboard"
                self._add_to_my_dashboard(record)
        return records

    def write(self, vals):
        result = super(DashboardChart, self).write(vals)
        # Track the user who modified the chart
        if self.env.user:
            for record in self:
                if self.env.user.id not in record.worked_by_user_ids.ids:
                    record.worked_by_user_ids = [(4, self.env.user.id)]
                    # Also add this chart to the user's "My Dashboard"
                    self._add_to_my_dashboard(record)
        return result

    def _add_to_my_dashboard(self, chart):
        """
        Add the chart to the user's "My Dashboard" if it exists
        """
        # Get or create the "My Dashboard" for the current user
        my_dashboard = self.env["dashboard.dashboard"].get_or_create_my_dashboard(self.env.user)
        
        # If we found a dashboard and this chart isn't already assigned to it
        if my_dashboard and chart.id not in my_dashboard.chart_ids.ids:
            chart.write({"dashboard_id": my_dashboard.id})
    ...
    # Track which users have worked on this chart
    worked_by_user_ids = fields.Many2many(
        "res.users",
        "chart_user_rel",
        "chart_id",
        "user_id",
        string="Worked By Users",
        help="Users who have worked on this chart",
    )
